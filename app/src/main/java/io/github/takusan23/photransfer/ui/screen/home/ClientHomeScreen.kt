...
fun ClientHomeScreen() {
    ...
    val latestUploadDate = dataStore.value?.get(SettingKeyObject.CLIENT_LATEST_UPLOAD_DATE)
    // サーバー検索
            ...
            Box(modifier = Modifier.padding(it)) {
                Column(modifier = Modifier.padding(start = 10.dp, end = 10.dp)) {
                    ...
                    if (findServer.value != null) {
                        ...
                        Spacer(modifier = Modifier.padding(top = 20.dp))
                        // 転送画面
                        ClientUploadOptionButton(latestUploadDate)
                    } else {
                        ServerNotFoundInfo()
                    }
 ...
 * */
@Composable
fun ClientEnableSwitch(isRunning: Boolean, setting: DataStore<Preferences>) {
    val scope = rememberCoroutineScope()
    val context = LocalContext.current
